...
def init_weights(shape):
    return tf.Variable(tf.random_normal(shape, stddev=0.0))
...
def model_2hidden(X, w_h, w_h2, w_o, b_h, b_h2, b_o, p_drop_input, p_drop_hidden): # this network is the same as the previous one except with an extra hidden layer + dropout
    ...
    h2 = tf.tanh(tf.matmul(h, w_h2) + b_h2)
...
Y = tf.placeholder("float", shape=[None, 1000])
...
w_o = init_weights([625, 1000])
b_o = init_weights([1000])
...
for i in range(250):
    ...
    print i, sess.run(cost, feed_dict={X: teX, Y: teX,
                                                     p_keep_input: 1.0,
                                                     p_keep_hidden: 1.0})
print "Expected output:"
print teX
print "Calculated output:"
print sess.run(py_x, feed_dict={X: teX, Y: teX,p_keep_input: 1.0,p_keep_hidden: 1.0})
